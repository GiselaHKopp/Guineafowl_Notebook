...
Information on dominance hierarchy of the mpala group is stored in *mpala_ranks.csv*.

```{r}
dominance <- read.csv("data/mpala_ranks.csv", header = TRUE, sep = ",", as.is = TRUE)

#remove first column of row numbers
dominance <- dominance[2:3]

#create dataframe with dominace differences between dyads

##create two ID columns
ID_1 <- dominance$ids_
ID_2 <- dominance$ids_

dominance_adjacency <- bind_cols(ID_1, ID_2)

names(dominance_adjacency) <- c("ID_1", "ID_2")

##generate all combinations of IDs
dyads <- tidyr::expand(dominance_adjacency, ID_1, ID_2)

##remove self to self dyads
dyads <- filter(dyads, ID_1 != ID_2)

##add column with dominance rank of ID_1
dyads <- left_join(dyads, dominance, by = c("ID_1"="ids_"))
dyads <- rename(dyads, c(rank_1="ranks_"))

##add column with dominance rank of ID_2
dyads <- left_join(dyads, dominance, by = c("ID_2"="ids_"))
dyads <- rename(dyads, c(rank_2="ranks_"))

##add column with rank difference of dyads
dyads$RankDiff <- abs(dyads$rank_1 - dyads$rank_2) #take absolute value to not have negatives

##add column with dyad
dyads$dyad <- paste(dyads$ID_1, dyads$ID_2, sep = "-")
...
```{r figure c-curve}
theme_set(dark_theme_bw()) #set theme to black and white
...
theme_set(dark_theme_classic()) #set theme to black and white
...
ggplot(data_overview, aes(x =  reads_post_trim_total, y = sequenced_library_complexity, color = original_reseq, shape = original_reseq)) +
  ...
  scale_colour_manual(values = two_colors_reverse) +
...
```{r coverage}
theme_set(dark_theme_classic()) #set theme to black background
...
ggplot(READ_results_sample_info, aes(x = reorder(PairIndividuals, -NonNormalizedP0), y = NonNormalizedP0, color = dyad_sex))  +
  ...
  geom_point(alpha = 0.4, aes(y = NonNormalizedP0)) + #plot relatedness estimates as points 
...
```{r correlation Nm Gg mapping ngsrelate}
...
filter(READ_results_sample_info, a_Group != "mpala_chicks", b_Group != "mpala_chicks")
...
ggplot(filter(relate_results, dyad_group == "intragroup", a_Group != "mpala_chicks", b_Group != "ROOP"), aes(x = dyad_sex, y = NonNormalizedP0, color = dyad_sex)) +
...
ggplot(subset(relate_results, dyad_group %in% "intragroup"), aes(x = dyad_sex, y = NonNormalizedP0, color = dyad_sex)) +
...
dominance_relatedness <- left_join(dyads, relate_results)

#only keep dyad, rank difference and relatedness values

dominance_relatedness <- select(dominance_relatedness, RankDiff, dyad, rab, pi_HAT, NonNormalizedP0, coverage)

#only keep dyads with existing relatedness values
dominance_relatedness <- filter(dominance_relatedness, !is.na(NonNormalizedP0))

str(dominance_relatedness)
...
ggplot(dominance_relatedness, aes(x = RankDiff, y = pi_HAT, color = RankDiff, size = coverage)) +
...
ggplot(relate_results, aes(x = coverage, y = NonNormalizedP0, colour = dyad_sex)) +
